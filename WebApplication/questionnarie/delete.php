...
<?php
    ...
    if ( !empty($_POST)) {
		...
		try{
			...
			/*
			$sql = "DELETE FROM question_linguistic_scale  WHERE quest_id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($id));
			*/
			
			
			$sql = "DELETE FROM questionnarie_user  WHERE quest_id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($id));
			...
			$sql = "DELETE FROM criteria_alternative  WHERE questionnaire_id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($id));
			
			$sql = "DELETE FROM questions  WHERE questionnaire = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($id));
			
			$sql = "DELETE FROM criteria  WHERE quest_id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($id));
			
			$sql = "DELETE FROM alternative  WHERE questionnaire_id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($id));
                ...
                <div class="span10 offset1">
                    ...
                    <form class="form-horizontal" action="delete.php" method="post">
                      ...
                      <div class="form-actions">
                          ...
                          <a class="btn" href="../insertQuestionnarie.php">No</a>
                        </div>
                    </form>
                </div>
            </div>     
